...
def board_to_tensor(board: chess.Board) -> torch.Tensor:
    """
    Converts a python-chess Board (object class built using bitboards) into an 19x8x8 numerical tensor (channel, height, width).
...
#   Check is a global facts about the side to move, so fill we can fill the whole channel with 1.0 if the side to move is in check, else 0.0
